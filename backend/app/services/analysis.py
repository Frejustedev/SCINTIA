...
def run_analysis(db: Session, *, study: Study) -> ExamScore | None:
    """Apply the exam-specific analyzer and store its score (if any)."""
    ...
    if result.score_type is None or result.score_value is None:
        return None
    score = ExamScore(
        study_id=study.id,
